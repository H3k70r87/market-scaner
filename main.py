...
For each configured asset + timeframe combination:
  ...
  3. If confidence > min_confidence and not duplicate: save alert + send Telegram message
"""
...
def run_scan(min_confidence: Optional[float] = None) -> list[dict]:
    """
    ...
    logger.info("Config: min_confidence=%.0f, cooldown=%dh, patterns=%s",
                min_confidence, cooldown_hours, enabled_patterns)
    logger.info("=" * 60)
